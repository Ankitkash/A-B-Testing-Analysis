...
st.markdown("**Upload your A/B test dataset and get a full statistical analysis instantly.**")
st.markdown("---")
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 📌 What is MDE?")
st.sidebar.info("The smallest improvement that is worth detecting. Smaller MDE = larger sample needed.")

# ── FILE UPLOAD ───────────────────────────────────────────────
st.header("📂 Step 1: Upload Dataset")
st.markdown("Download dataset from 👉 [Kaggle — zhangluyuan/ab-testing](https://www.kaggle.com/datasets/zhangluyuan/ab-testing)")
...
st.success(f"✅ Data cleaned — {removed:,} mismatched/duplicate rows removed. {len(df_clean):,} rows remaining.")
...
if p_srm < 0.05:
    st.error("⚠️ SRM DETECTED — Investigate randomization before trusting results.")
else:
    st.success("✅ No SRM — Split looks good (close to 50/50).")
...
| H₁ | p_control ≠ p_treatment (two-tailed) |
""")
...
ax.grid(True, axis='x', alpha=0.3); plt.tight_layout(); st.pyplot(fig3)

if ci_low > 0 or ci_high < 0:
    st.success("✅ CI does NOT contain 0 — Statistically Significant")
else:
...
mde_range = np.linspace(0.001, 0.05, 200)
...
ax.legend(); ax.grid(True, alpha=0.3); plt.tight_layout(); st.pyplot(fig4)
